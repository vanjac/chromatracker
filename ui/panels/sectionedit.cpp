...
void SectionEdit::draw(App *app, Rect rect, shared_ptr<Section> section)
{
    ...
    if (tempo != Section::NO_TEMPO) {
        if (auto act = tempoSpinner.draw(app, tempoR, &tempo, 20, 999, 1.0/2);
                (bool)act) {
            app->undoer.doOp(edit::ops::SetSectionTempo(section, tempo), act);
        }
        if (tempoButton.draw(app, clearR, C_DARK_GRAY)) {
            app->undoer.doOp(edit::ops::SetSectionTempo(
                section, Section::NO_TEMPO));
        }
        drawText("X", clearR(TL), C_WHITE)(TR);
    } else {
        if (tempoButton.draw(app, tempoR, C_DARK_GRAY)) {
            app->undoer.doOp(edit::ops::SetSectionTempo(section, 125));
        }
    ...
    if (meter != Section::NO_METER) {
        if (auto act = meterSpinner.draw(app, meterR, &meter, 1, 32, 1.0/15);
                (bool) act) {
            app->undoer.doOp(edit::ops::SetSectionMeter(section, meter), act);
        }
        if (meterButton.draw(app, clearR, C_DARK_GRAY)) {
            app->undoer.doOp(edit::ops::SetSectionMeter(
                section, Section::NO_METER));
        }
        drawText("X", clearR(TL), C_WHITE)(TR);
    } else {
        if (meterButton.draw(app, meterR, C_DARK_GRAY)) {
            app->undoer.doOp(edit::ops::SetSectionMeter(section, 4));
        }
