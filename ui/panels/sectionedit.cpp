...
void SectionEdit::draw(App *app, edit::Editor *editor,
                       Rect rect, shared_ptr<Section> section)
{
    ...
    if (tempo != Section::NO_TEMPO) {
        if (tempoSpinner.draw(app, editor, tempoR, &tempo, 20, 999, 1.0/2)) {
            editor->undoer.doOp(edit::ops::SetSectionTempo(section, tempo),
                                true);
        }
        if (tempoButton.draw(app, clearR, C_DARK_GRAY)) {
            editor->undoer.doOp(edit::ops::SetSectionTempo(
                section, Section::NO_TEMPO), true);
        }
        drawText("X", clearR(TL), C_WHITE)(TR);
    } else {
        if (tempoButton.draw(app, tempoR, C_DARK_GRAY)) {
            editor->undoer.doOp(edit::ops::SetSectionTempo(section, 125), true);
        }
    ...
    if (meter != Section::NO_METER) {
        if (meterSpinner.draw(app, editor, meterR, &meter, 1, 32, 1.0/15)) {
            editor->undoer.doOp(edit::ops::SetSectionMeter(section, meter),
                                true);
        }
        if (meterButton.draw(app, clearR, C_DARK_GRAY)) {
            editor->undoer.doOp(edit::ops::SetSectionMeter(
                section, Section::NO_METER), true);
        }
        drawText("X", clearR(TL), C_WHITE)(TR);
    } else {
        if (meterButton.draw(app, meterR, C_DARK_GRAY)) {
            editor->undoer.doOp(edit::ops::SetSectionMeter(section, 4), true);
        }
